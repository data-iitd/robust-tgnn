...
class StaticAttentionLayer(torch.nn.Module):
  """
  Static attention layer. Return the static embedding of a node given the node itself,
   its neighbors and the edge features.
  """
  ...
  def __init__(self, n_node_features, n_neighbors_features, n_edge_features,
    ...
    self.key_dim = n_neighbors_features + n_edge_features
  ...
  def forward(self, src_node_features, neighbors_features,
               edge_features, neighbors_padding_mask):
    """
    "Temporal attention model
    :param src_node_features: float Tensor of shape [batch_size, n_node_features]
    :param src_time_features: float Tensor of shape [batch_size, 1, time_dim]
    ...
    #query = torch.cat([src_node_features_unrolled, src_time_features], dim=2)
    key = torch.cat([neighbors_features, edge_features], dim=2)
