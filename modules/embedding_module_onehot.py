...
from model.temporal_attention import TemporalAttentionLayer,StaticAttentionLayer
...
class GraphEmbedding(EmbeddingModule):
  def __init__(self, node_features, edge_features, memory, neighbor_finder, time_encoder, n_layers,
    ...
    self.device = device
    self.transform_one_hot = nn.Linear(node_features.shape[1],172)
  ...
  def compute_embedding(self, memory, source_nodes, timestamps, n_layers, n_neighbors=20, time_diffs=None,
    ...
    source_node_features = self.transform_one_hot(self.node_features[source_nodes_torch, :])
...
#### new class added for static graph attention
class StaticGraphAttentionEmbedding(GraphEmbedding):
  def __init__(self, node_features, edge_features, memory, neighbor_finder, time_encoder, n_layers,
               n_node_features, n_edge_features, n_time_features, embedding_dimension, device,
               n_heads=2, dropout=0.1, use_memory=True):
    super(StaticGraphAttentionEmbedding, self).__init__(node_features, edge_features, memory,
                                                  neighbor_finder, time_encoder, n_layers,
                                                  n_node_features, n_edge_features,
                                                  n_time_features,
                                                  embedding_dimension, device,
    ...
    self.attention_models = torch.nn.ModuleList([StaticAttentionLayer(
      n_node_features=n_node_features,
      n_neighbors_features=n_node_features,
      n_edge_features=n_edge_features,
      n_head=n_heads,
...
def get_embedding_module(module_type, node_features, edge_features, memory, neighbor_finder,
  ...
  if module_type == "graph_attention":
    ...
    return TimeEmbedding(node_features=node_features,
      ...
      return StaticGraphAttentionEmbedding(node_features=node_features,
                                  edge_features=edge_features,
                                  memory=memory,
                                  neighbor_finder=neighbor_finder,
                                  time_encoder=time_encoder,
